...
import { atomSelected, elemento, tiempo, oxi, descomponer, dummies } from "./interactive.js";
...
function eliminar(event) {
    if (event.clientX >= (window.innerWidth * 0.81)) {
        if (elemento[atomSelected].elemento) {
        ...
        dummies[atomSelected].children.forEach(element => {
            if(element.userData.child){
                scene.remove(element.userData.child);
            }

            dummies[atomSelected].remove(element);
        });
        scene.remove(dummies[atomSelected]);
